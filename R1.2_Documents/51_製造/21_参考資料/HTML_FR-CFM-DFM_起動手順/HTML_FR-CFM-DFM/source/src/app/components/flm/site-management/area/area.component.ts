...
import { GroupAreaMap } from '../../../../types/flm/site-management';
...
export class AreaComponent extends KbaAbstractBaseComponent {
  thList: TableHeader[];
  list: GroupAreaMap[];
  filteredList: GroupAreaMap[];
  layout = 'list';
  storage: KbaStorageObject;
  detailLink = '/site-management/detail/stat/area/';
   ...
   * @param id 顧客ID
   */
  async handleCustomerIDSelect(id) {
    await this._fetchList(id);
  }
  ...
  protected _createThList(fields: Fields) {
    ...
    return thList.map(th => ({
      ...th,
      link: th.name === 'group_area_maps.label' ? this.detailLink : null,
    }));
  }
  ...
  private async _fetchDataForInitialize() {
    const res = await this.siteManagementService.fetchAreaInitData();
    ...
    this.requestHeaderParams['X-Fields'] = this._createXFields(res.fields).join(
      ','
    );
    this.thList = this._createThList(res.fields);
    // 顧客ユーザー時
    if (!this.resource.customer_ids) {
      this._fetchList();
    }
    this.onLoad();
  }
   ...
   * @param id 顧客ID
   */
  private async _fetchList(id?: string) {
    const params = id ? { customer_ids: [id] } : null;
    this.list = (await this.siteManagementService.fetchAreaMaps(
      params,
      this.requestHeaderParams
    )).result_data.group_area_maps;
