...
@Component({
  ...
  styleUrls: ['./subgroup-detail.component.scss'],
})
export class SubgroupDetailComponent extends KbaAbstractIndexComponent {
  @Input() labels;
  @Input() resource;
  @Input() functions;
  ...
  async fetchList(sort_key?: string) {
    ...
    const res = await this.subgroupService.fetchCarListData(
      {
        common: {
          user_permission_sub_group_ids: [
            this.detailData['sub_group.identification.id'],
          ],
        },
      },
