...
import { Labels, TableHeader } from '../../types/common';
...
export class KbaModalService {
  ...
  open(option: KbaModalOption, modalOption: NgbModalOptions = {}) {
    ...
    this.modalRef = this.modalService.open(KbaModalComponent, {
      ...modalOption,
   ...
   * @param modalOption モーダルオプション
   */
  customOpen(modal, option: any, modalOption: NgbModalOptions = {}): void {
    this._beforeOpen(option, modalOption);
   ...
   * @param responseData レスポンスデータ
   */
  createResultCountMessage(labels: Labels, responseData): string {
   ...
   * @param resource リソース
   */
  private _createResultVal(requestData, responseData, resource): any[] {
    const resultVal = [];
   ...
   * @param resource リソース
   */
  private _buildErrorMessages(errorData: ErrorData, resource): string[] {
   ...
   * @param resource リソース
   */
  private _replacePath(message: string, keys: string[], resource): string {
