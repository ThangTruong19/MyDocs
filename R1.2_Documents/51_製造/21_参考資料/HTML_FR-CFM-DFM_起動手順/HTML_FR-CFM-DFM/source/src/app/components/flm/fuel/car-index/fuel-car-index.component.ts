...
@Component({
  selector: 'app-fuel-car-index',
  templateUrl: './fuel-car-index.component.html',
  styleUrls: ['./fuel-car-index.component.scss'],
})
export class FuelCarIndexComponent extends KbaAbstractIndexComponent
  implements OnInit {
  ...
  stringParamList = [
    ...
    'accumulate_fuel_interval_item.id',
  ...
  lineBreakColumns = [
    'cars.accumulate_fuel_interval_items.management_item_name',
  ...
  async fetchList(sort_key?: string): Promise<any> {
    ...
    const params = _.cloneDeep(this._searchParams);
    _.unset(params, 'common.service_distributor.organization_codes');
    _.unset(params, 'common.support_distributor.organization_codes');
    const res = await this.fuelService.fetchCarsFuelList(
      params,
  ...
  onClickSearch() {
    ...
    this.fuelService.updateCarSearchCondition(
  ...
  onClickReset() {
    this.modalService.open({
      ...
      ok: () => {
        this.fuelService.initCarSearchCondition();
        this._resetResource();
  ...
  protected async _fetchDataForInitialize(): Promise<any> {
    const res = await this.fuelService.fetchFuelCarIndexInitData();
    this.initialize(res);
    this.params = _.cloneDeep(this.initParams);
  ...
  /**
   * 配列で返ってくるものを一覧用にデータを整形する
   * @param data 対象のデータ
   * @param th 対象のヘッダー
   */
  protected _listDisplayData(data, th) {
    if (
      th.formatKey === 'accumulate_fuel_interval_items.management_item_name'
    ) {
      const fuelItems = _.get(data, 'accumulate_fuel_interval_items');
      return fuelItems.map(item => item.management_item_name);
    } else {
  ...
  protected _getResourceValueName(path: string, value: string): string {
    ...
    if (res) {
      const val = _.find(res.values, item => item.value === value);
      return val ? val.name : '';
    } else {
      return '';
    }
  ...
  private _addSelectParams(_searchParams, selectParams) {
    ...
    return _.mergeWith({}, _searchParams, params, customizer);
  }
  ...
  private async _downloadTemplate(fields, accept) {
    ...
    try {
      ...
      const res = await this.fuelService.createCarFile(
        {
          ...p,
  ...
  private _resetResource() {
    const resourcePahts = this.belongResourcePaths.concat([
  ...
  private _buildSearchParams(nestedKeys: string[]) {
    const params = this._transrateSearchParams(this.params, nestedKeys);
    if (this.modelCarTypeCheck) {
      _.each(
        ...
        key => _.unset(params, key)
      );
    } else {
      _.unset(params, 'common.car_identification.model_type_revs');
    }
  ...
  */
  private _buildDownloadFields() {
    return new Promise<void>(async (resolve) => {
      const res = await this.fuelService.fetchCarIndexDownloadFields();
  ...
  private _buildFieldResources() {
    return new Promise<void>(async (resolve) => {
      const res = await this.fuelService.fetchCarIndexFieldResouces();
  ...
  private _buildDownloadFieldResources() {
    return new Promise<void>(async (resolve) => {
      const res = await this.fuelService.fetchCarIndexDownloadFieldResources();
