...
@Directive({
  selector: '[appKbaSortingLabel]',
})
export class KbaSortingLabelDirective implements OnInit {
  ...
  @HostBinding('class.KBA-sortable-label-asc')
  get labelAsc(): boolean {
    if (!this._isValid()) {
      return false;
    }
  ...
  @HostBinding('class.KBA-sortable-label-desc')
  get labelDesc(): boolean {
    if (!this._isValid()) {
      return false;
    }
  ...
  private _sortByMultipleKeys(): void {
    this.sortingParams['sortLabel'] = this.sortingParams['sortLabel'].map(p => {
