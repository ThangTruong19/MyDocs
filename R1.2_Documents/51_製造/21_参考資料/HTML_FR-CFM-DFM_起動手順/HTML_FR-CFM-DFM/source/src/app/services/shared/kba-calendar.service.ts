...
import { MonthPickerParams, DatePickerParams } from '../../types/calendar';
...
@Injectable()
export abstract class KbaCalendarService {
  ...
  set baseDay(baseDay: Moment) {
    this._baseDay = baseDay;
  }

  initialize(params: MonthPickerParams | DatePickerParams) {
   ...
   */
  toMoment(dateString: string = '', clearHours = true): Moment {
   ...
   * @param labels ラベル
   */
  getMonthLabels(labels): MonthLabels {
    return [
   ...
   * カレンダー用の日付を生成する
   */
  abstract createCalendar(): CommonMonthPickerMonth[] | CommonDatePickerDay[];
   ...
   */
  abstract getInputText(day: Moment, dateFormat: string): string;
   ...
   * @returns 日付フォーマットに従った「年月日」
   */
  abstract inputDateFormat(dateFormat: string): string;
