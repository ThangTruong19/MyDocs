...
function startServer(fn) {
  ...
  if (options.has('opa')) {
    ...
    server.listen(4301, function () {
      console.log('OPA Api mock server is listening on port 4301');
    });
  } else if (options.has('komtrax-link')) {
    var server = http.createServer(app.use(apimock('data/apimock.komtrax-link.yml')));
    server.listen(4301, function () {
      console.log('KOMTRAX-LINK Api mock server is listening on port 4301');
    });
  } else {
    var server = http.createServer(app.use(apimock('data/apimock.yml')));

    server.listen(4301, function () {
      console.log('FLM Api mock server is listening on port 4301');
    });
  }
}
...
function setEnvironment(app, setting) {
  if (process.platform === 'win32') {
    ...
    //windows以外
    execSync(`cp .angular-cli_${app}.json angular.json`);
  ...
  if (process.argv.includes('--replace-api-host') || process.argv.includes('-h') && setting === 'local') {
    ...
    fs.writeFileSync(settingsPath, settings.replace(/hostname:\s*'[\w\d\.]+:4301'/, `hostname: '${addr}:4301'`));
  }
}
...
gulp.task('build:setup:flm', function () {
  return setupBuild('flm');
});

gulp.task('build:setup:opa', function () {
  return setupBuild('opa');
});

gulp.task('build:setup:kom', function () {
  return setupBuild('komtrax-link');
});
...
gulp.task('build:flm', ['build:setup:flm', 'scripts:maplibrary', 'scripts:jquery'], function () {
  return build('flm');
});

gulp.task('build:opa', ['build:setup:opa', 'scripts:maplibrary', 'scripts:jquery'], function () {
  return build('opa');
});

gulp.task('build:komtrax-link', ['build:setup:kom', 'scripts:maplibrary', 'scripts:jquery'], function () {
  return build('komtrax-link');
});

gulp.task('env:local:flm', function () {
  return setEnvironment('flm', 'local');
});

gulp.task('env:local:opa', function () {
  return setEnvironment('opa', 'local');
});

gulp.task('env:local:komtrax-link', function () {
  return setEnvironment('komtrax-link', 'local');
});

gulp.task('env:dfm', function () {
  return setEnvironment('flm', 'dfm');
});

gulp.task('env:cfm', function () {
  return setEnvironment('flm', 'cfm');
});

gulp.task('env:opa', function () {
  return setEnvironment('opa', 'opa');
});

gulp.task('env:komtrax-link', function () {
  return setEnvironment('komtrax-link', 'dfm');
});
