...
import { AppModalComponent } from 'app/components/customize-setting-upload/upload-result/app-modal.component';

import { environment } from 'environments/environment';

export interface KbaModalOption {
  title: string;
  labels: Labels;
...
@Injectable()
export class KbaModalService {
  CLOSE_TIMEOUT = 300;

  modalRef: NgbModalRef;
  modalStack: NgbModalRef[] = [];
  private _currentScrollY: any;
  private _enableOk: boolean;

  set enableOk(value: boolean) {
   ...
   */
  open(option: KbaModalOption, modalOption: NgbModalOptions = {}) {
    this._beforeOpen(option, modalOption);
    this.modalRef = this.modalService.open(AppModalComponent, {
      ...modalOption,
      size: modalOption.size || 'lg',
    });//alert(option.ok);alert(option.showCloseBtn);
    this.modalRef.componentInstance.title = option.title;
    this.modalRef.componentInstance.labels = option.labels;
    this.modalRef.componentInstance.content = option.content;console.log('content?');console.log(option.content);
    ...
    this.modalRef.componentInstance.showCloseBtn = !_.isUndefined(
      ...
      ? option.showCloseBtn
      : true;
    this.enableOk = !_.isUndefined(option.enableOk) ? option.enableOk : true;console.log('huh?');console.log(this.modalRef.result);
   ...
   * @param modalOption モーダルオプション
   */
  customOpen(modal: any, option: any, modalOption: NgbModalOptions = {}): void {
    this._beforeOpen(option, modalOption);
  ...
  /**
   * 指定したヘッダ情報からモーダル用に必要カラムをピックアップした新しいヘッダ情報を返す
   * @param ths ヘッダ情報
   * @param columns 必要カラム
   */
  pickModalDesc(ths: any, columns: any) {
   ...
   * @param columns 必要カラム
   */
  pickModalVal(dataList: any, columns: any) {
    return _.reduce(
      dataList,
      (array, data) => {
  ...
  createResultModalResource(
    ...
    const resultCountMessage = this.createResultCountMessage(
      labels,
      responseData
    );
   ...
   * @param responseData レスポンスデータ
   */
  createResultCountMessage(labels: Labels, responseData: any): string {
    const total = responseData.length;
    const success = _.filter(responseData, res => {
   ...
   * モーダルを開く時の前処理
   */
  private _beforeOpen(option: KbaModalOption, modalOption: NgbModalOptions) {
   ...
   * モーダルを開く時の後処理
   */
  private _afterOpen(modalRef: NgbModalRef) {alert('yo');
    if (modalRef.componentInstance) {alert(this.modalStack.length);
    ...
    setTimeout(() => {
      ...
      this.modalStack.push(modalRef);alert('push '+this.modalStack.length);console.log('pushed modalRef = ');console.log(modalRef);
    }, 200);
  }
   ...
   */
  private _afterClose() {
    this.modalStack.pop();alert('after close '+this.modalStack.length);
    if (this.modalStack.length === 0) {
      // モーダル背景の固定を解除
      $('body').attr({ style: '' });
  ...
  private getModalElements(index: number): Element[] {
    ...
    console.log('modalelements');console.log(modal);
    if (modal == null) {
      return [];
    }
