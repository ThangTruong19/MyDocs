 ...
 */
@Injectable()
export class DatePickerService extends CalendarService {
    private _initialBaseDay: string;

    public get initialBaseDay() {
        return this._initialBaseDay;
    }

    public set initialBaseDay(initialBaseDay: string) {
     ...
     */
    public isLastMonth(day: Moment): boolean {
     ...
     */
    public isNextMonth(day: Moment): boolean {
     ...
     * カレンダー用の日付を生成する
     */
    public createCalendar(): CommonDatePickerDay[] {
        let startDay;
        switch (true) {
        ...
        const days: CommonDatePickerDay[] = [];
     ...
     */
    public convertDays(): CommonDatePickerDay[][] {
        return reduce(
            this.createCalendar(),
            (array, day, idx) => {
     ...
     * @returns 日付フォーマットに従った「年月日」
     */
    public getInputDatetimeText(day: Moment, dateFormat: string): string {
     ...
     * ユーザ設定の日付フォーマットを返却する
     * @param dateFormat 日付フォーマット
     * @returns 日付フォーマットに従った「年月日 + 時分秒」
     */
    public inputDateTimeFormat(dateFormat: string): string {
        switch (dateFormat) {
            case DateTimeFormatKind.yearFirst:
