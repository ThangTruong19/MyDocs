...
import { AuthoritySelectCheckboxDirective } from './authority-select-checkbox.directive';
...
@Component({
  template: `
    ...
    <div>
      <input
        (appAuthoritySelectCheckbox)="onChange($event)"
...
class AuthoritySelectCheckboxDirectiveTestComponent {
  selectedList: any[] = [];
  data: any[] = ['1', '2', '3'];
  checkAll = false;
...
describe('AuthoritySelectCheckboxDirective', () => {
  let component: AuthoritySelectCheckboxDirectiveTestComponent;
  let fixture: ComponentFixture<
    AuthoritySelectCheckboxDirectiveTestComponent
  >;
  let checkAllEl: any;
  let checkboxEl1: any;
  let checkboxEl2: any;
    ...
    describe('全選択/解除について', () => {
      ...
      it('全解除した場合 selectedList が空になっていること & 全解除されていること', async(() => {
        fixture.whenStable().then(() => {
          checkAllEl.click();
          fixture.detectChanges();
          const _selectedList: jasmine.Expected<jasmine.ArrayLike<any>> = [];
          expect(component['selectedList']).toEqual(_selectedList);
          expect(checkboxEl1.checked).not.toBeTruthy();
