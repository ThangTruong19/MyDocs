...
import { CustomSelectItem } from 'app/constants/form';
...
export class SelectedComponent implements OnInit {
    ...
    @Input() public hasEmpty = false;
    @Input() public notEditable = false;
    @Input() public scrollTop = 0;
    @Input() public appendTo?: string;

    @Output() public onChangeSelectItem: EventEmitter<any> = new EventEmitter<any>();
    ...
    private _getInitialItem(): SelectItem {
        return (
            this.items.find((item: SelectItem) => item.id === this.itemParams[this.itemName]) ||
            this.items.find((item: SelectItem) => item.isHighlightItem) ||
            this.items[0]
        );
    }
    ...
    private _getItems(): SelectItem[] {
        ...
        return items.concat(
            target.values.map((val: ResourceValue) => ({
