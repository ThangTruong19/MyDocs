...
import { Item, GetHistoryMgtListRequestParam, PrintHistoryMgtListRequestParam, ApiResult, InitializeApiResult } from 'app/types/history-mgt-list';
import { Fields, Labels, Resource, FieldSelectValue, DownloadValue } from 'app/types/common';
...
export class HistoryMgtListComponent extends AbstractIndexComponent {
    ...
    protected belongingCategoryCode: SelectedComponent;
    ...
    protected downloadFields: Fields;
    protected downloadFieldResources: Fields;
    protected fieldSelectPopoverVisible = false;
    protected downloadPopoverVisible = false;
    protected _dateFormat = '';
    protected timeZone = '';
    protected enableDateRange: string[] = [];
    protected beginningWday: number;
    protected excludeSearchParams: string[] = ['date_from_formatted', 'date_to_formatted'];
    protected datePickerLabels: Labels;
    protected override commaSeparated: string[] = ['model', 'type_rev', 'serials'];
    ...
    protected async fetchList(sort_key?: string): Promise<void> {
        ...
        const params: GetHistoryMgtListRequestParam = {
            operation_history: _.omit(this.searchParams, this.excludeSearchParams),
        };
        const res: ApiResult = await this.historyMgtListService.fetchIndexList(params, this.requestHeaderParams);
        const list: Item[] = this._formatList(res.result_data.operation_histories, this.thList);
     ...
     * 表示項目設定ボタン押下時の処理
     */
    protected onClickFieldSelect(): void {
     ...
     * 一括ダウンロードボタン押下時の処理
     */
    protected onClickDownloadAll(): void {
    ...
    protected async onCustomizeUsageDefinitionIdChange(value: string): Promise<void> {
        ...
        this.resource.operation_history.customize_definition_id = res.operation_history.customize_definition_id;
     ...
     * @param event イベント
     */
    protected async onFieldSelectOk(event: FieldSelectValue): Promise<void> {
        await this.api.updateField(FunctionCodeConst.HISTORY_MGT_LIST_FUNCTION, event.fields);
        const res: Fields = await new Promise((resolve) =>
    ...
    protected async _fetchDataForInitialize(): Promise<void> {
        const res: InitializeApiResult = await this.historyMgtListService.fetchIndexInitData();
        this.initialize(res);
     ...
     */
    protected async _downloadTemplate(fields: string[], accept: string): Promise<void> {
        const params: PrintHistoryMgtListRequestParam = {
            operation_history: _.omit(this.searchParams, this.excludeSearchParams),
        ...
        try {
            const res: ApiResult = await this.historyMgtListService.createFile(params, header);
