window.settings = {
    ...
    useEntranceForDevelop: true, // ローカル開発用の追加（開発用のダミーのエントランス機能を有効にする）
    develop: {
        accessToken: 'dummy123', // ローカル開発用のダミートークン（リリース時は削除すること）
        groupId: '2', // ローカル開発用のダミーグループID（リリース時は削除すること）
        userId: 'DevUser001@dev.com', // ローカル開発用のダミーユーザID（リリース時は削除すること）
        userName: 'DevUser001' // ローカル開発用のダミーユーザ名（リリース時は削除すること）
    }
};
