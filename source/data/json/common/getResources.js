  ...
  return new Promise(function (resolve) {
    ...
    if (data.body.search_parameters) {
      ...
      if (_.has(params, 'condition_sets')) {
        ...
        switch (screenCode) {
          case 'cdsm_car_mgt_regist': // 車両管理 登録画面
            // 使用国リソース取得の場合
...
function resolveConditionSets(resolve, conditionSets, screenCode, target) {
  ...
  switch (screenCode) {
    case 'cdsm_authority_mgt_list':
    case 'cdsm_history_mgt_list':
    case 'cdsm_customize_request_status_list':
    case 'cdsm_customize_setting_upload_list':
  ...
  switch (cond.condition) {
    ...
    case 'common.support_distributor.ids':
      ...
      switch (screenCode) {
        case 'cdsm_car_mgt_list': // 車両管理 一覧画面
          type = 'car_belong';
          break;
      }
